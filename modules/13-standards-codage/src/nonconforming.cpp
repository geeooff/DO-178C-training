...
static u32 g_last_result = 0U;

namespace mod13_nonconforming {

// V04 -- recursion.
//        Profondeur de pile non triviale a borner (module 08).
static u32 power_of_ten(u32 exponent) {
    if (exponent == 0) {
        return 1;
    }
    return 10 * power_of_ten(exponent - 1);  // V04
}
...
u32 bcd_to_binary(u16 bcd_word) noexcept {
    ...
    for (i = 0; i < 4; i++) {
        ...
        u32 digit = (u32)DIGIT((u32)bcd_word, i);
        ...
        result += digit * power_of_ten((u32)i);
    }
...
static u32 unused_conversion(u16 word) {
    return (u32)word * 2U;
}
