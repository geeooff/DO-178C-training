...
//  Enumerations
// -----------------------------------------------------------------------------
TEST_REQ(Enum, valid_nominal_values, "LLR-M01-001") {
...
TEST_REQ(Enum, robustness_value_out_of_domain, "LLR-M01-002") {
...
TEST_REQ(Enum, name_of_each_member, "LLR-M01-003") {
...
TEST_REQ(Enum, underlying_type_is_a_byte, "LLR-M01-004") {
...
// -----------------------------------------------------------------------------
TEST_REQ(Saturation, addition_nominal_domain, "LLR-M01-010") {
...
TEST_REQ(Saturation, addition_at_bounds, "LLR-M01-011") {
...
TEST_REQ(Saturation, subtraction_at_bounds, "LLR-M01-012") {
...
TEST_REQ(Saturation, multiplication_at_bounds, "LLR-M01-013") {
...
TEST_REQ(Saturation, compile_time_evaluation, "LLR-M01-014") {
...
// -----------------------------------------------------------------------------
TEST_REQ(Checked, addition_nominal, "LLR-M01-020") {
    i32 result = 0;
    CHECK(mod01::checked_add(2000000000, 100, result));
    CHECK_EQ(result, 2000000100);
}

TEST_REQ(Checked, addition_overflow_positive, "LLR-M01-021") {
...
TEST_REQ(Checked, addition_overflow_negative, "LLR-M01-022") {
...
TEST_REQ(Checked, division_by_zero, "LLR-M01-023") {
    i32 result = 42;
    CHECK_FALSE(mod01::checked_div(100, 0, result));
    CHECK_EQ(result, 0);
}

TEST_REQ(Checked, division_min_by_minus_one, "LLR-M01-024") {
    constexpr i32 kMin = std::numeric_limits<i32>::min();
    i32 result = 42;
...
//  Conversions
// -----------------------------------------------------------------------------
TEST_REQ(Cast, narrowing_valid, "LLR-M01-030") {
    u8 small = 0U;
    CHECK(mod01::checked_cast<u8>(i32{200}, small));
    CHECK_EQ(small, u8{200U});
}

TEST_REQ(Cast, narrowing_rejected, "LLR-M01-031") {
...
TEST_REQ(Cast, signed_to_unsigned_negative_rejected, "LLR-M01-032") {
...
TEST_REQ(Cast, unsigned_to_signed_too_large_rejected, "LLR-M01-033") {
    i16 destination = 7;
...
TEST_REQ(Cast, widening_always_valid, "LLR-M01-034") {
    i32 large = 0;
...
//  Intervalles
// -----------------------------------------------------------------------------
TEST_REQ(Range, bounds_included, "LLR-M01-040") {
...
TEST_REQ(Range, clamping, "LLR-M01-041") {
...
//  Disposition memoire
// -----------------------------------------------------------------------------
TEST_REQ(Layout, observable_padding, "LLR-M01-050") {
