...
TEST_REQ(CycleDeVie, destruction_en_ordre_inverse, "LLR-M03-001") {
    ...
    for (usize index = 0U; index < 3U; ++index) {
        REQUIRE(mod03::LifetimeLog::entry(index, evenement, tag));
        CHECK_EQ(evenement, Event::Construct);
        CHECK_EQ(tag, static_cast<i32>(index) + 1);
    }
    // Destructions : 3, 2, 1 -- ordre INVERSE, garanti par la norme.
    for (usize index = 0U; index < 3U; ++index) {
        REQUIRE(mod03::LifetimeLog::entry(3U + index, evenement, tag));
        CHECK_EQ(evenement, Event::Destroy);
...
TEST_REQ(CycleDeVie, auto_affectation_sans_degat, "LLR-M03-004") {
    ...
    mod03::Traced& alias = objet;
    objet = alias;
    CHECK_EQ(objet.tag(), 5);

    mod03::Traced autre(6);
    autre = std::move(alias);
    CHECK_EQ(autre.tag(), 5);
}
...
TEST_REQ(RAII, acquisition_et_liberation_automatiques, "LLR-M03-010") {
    ...
    CHECK_EQ(mod03::DeviceBank::acquired_count(), u8{0});
    {
        ...
        REQUIRE(poignee.is_valid());
        CHECK_EQ(mod03::DeviceBank::acquired_count(), u8{1});
        CHECK(mod03::DeviceBank::is_acquired(poignee.channel()));
    }
...
TEST_REQ(RAII, liberation_explicite_idempotente, "LLR-M03-012") {
    reinitialiser();
    mod03::ChannelHandle poignee;
    REQUIRE(poignee.is_valid());

    poignee.release();
    CHECK_FALSE(poignee.is_valid());
    CHECK_EQ(mod03::DeviceBank::acquired_count(), u8{0});

    poignee.release();  // deuxieme appel : sans effet, pas de double liberation
    CHECK_EQ(mod03::DeviceBank::total_releases(), u32{1});
}
...
TEST_REQ(SectionCritique, imbrication, "LLR-M03-021") {
    reinitialiser();
    {
        const mod03::CriticalSection externe;
        {
            const mod03::CriticalSection interne;
            CHECK_FALSE(mod03::InterruptState::enabled());
        }
